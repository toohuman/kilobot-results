...
def main(args):
		...
		for r, radius in enumerate(RADII):
			...
			for cmp_i, cmp_dir in enumerate(RESULTS_DIRS):
				...
				for cmp_vi in range(len(collective_payoff_stats[r][cmp_i])):
					...
					legends.append(mlines.Line2D([], [], color=c[0], linewidth = line_width, linestyle=linestyles[line_i], label=CMP_RATES[r][cmp_i][cmp_vi]))
			...
			plt.savefig("trajectories" + os.sep + FIG_TITLE + "_" + str(radius) + GRAPHS_EXT, bbox_inches='tight')
			plt.clf()
	...
	for files in GRAPH_FILES:
		...
		plt.savefig("steadystates" + os.sep + FIG_TITLE + GRAPHS_EXT, bbox_inches='tight')
		plt.clf()
